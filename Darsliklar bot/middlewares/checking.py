...
class Asosiy(BaseMiddleware):
    async def on_pre_process_update(self,update:types.Update,data:dict):
        ...
        final_status = True
        for channel in Kanalar:
            status = await azo_bolish.azolik(user_id=user,channel=channel)
            ...
            if not status:
                ...
                await update.message.answer(text="Botdan foydalanish uchun quyidagi kanalarga a'zo bolishingiz shart!",reply_markup=button)
        if not final_status:
